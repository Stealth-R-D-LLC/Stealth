...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Stealth version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  ...
                  "  StealthCoind [options] help                " + _("List commands") + "\n" +
                  "  StealthCoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "StealthCoin:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -maxsendbuffer=<n>     " + _("Maximum per-connection send buffer, <n>*1000 bytes (default: 4000)") + "\n" +
#ifdef USE_UPNP
#if USE_UPNP
