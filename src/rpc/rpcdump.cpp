...
Value importprivkey(const Array& params, bool fHelp)
{
    ...
    // ppcoin: no importprivkey in mint-only mode
    ...
    CKeyID vchAddress = key.GetPubKey().GetID();
    {
        ...
        pwalletMain->ScanForWalletTransactions(pmemIndexGenesisBlock, true);
...
Value importaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
    {
        throw runtime_error(
            ...
            "Adds an address that can be watched as if it "
...

    {
        ...
        if (fRescan)
        {
            pwalletMain->ScanForWalletTransactions(pmemIndexGenesisBlock,
                                                   true);
            pwalletMain->ReacceptWalletTransactions();
        }
    }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    // ppcoin: no dumpprivkey in mint-only mode
    if (fWalletUnlockMintOnly)
    {
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
                           "Wallet is unlocked for minting only.");
