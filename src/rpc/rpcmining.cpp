...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("genproclimit",  (int)GetArg("-genproclimit", (int64_t) -1)));
...
Value GetNetworkHashPS(int lookup)
{
    ...
    int nHeightLookup = pindexBest->nHeight - lookup;

    CMapBlockLookup::const_iterator it = mapBlockLookup.find(nHeightLookup);

    CBlockMemIndex* pmemIndexPrev = pmemIndexGenesisBlock;
    if (it == mapBlockLookup.end())
    {
        printf("GetNetworkHashPS() : TSNH height %d is not in lookup\n",
               nHeightLookup);
    }
    else
    {
        pmemIndexPrev = it->second;
    }


    double timeDiff = pindexBest->GetBlockTime() -
                      GetMemIndexTime("GetNetworkHashPS", pmemIndexPrev);
    double timePerBlock = timeDiff / lookup;

    return (boost::int64_t) (((double) GetDifficulty() * pow(2.0, 32)) /
                             timePerBlock);
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        static int64_t nStart = numeric_limits<int64_t>::max();
    ...
    else
    {
        ...
        //     throw JSONRPCError(-100, "Unable to sign block, wallet
        //     locked?");

        {
            boost::lock_guard<QPRegistry> guardRegistry(*pregistryMain);

            return CheckWork(pblock, *pwalletMain, reservekey, pmemIndexBest);
        }
    }
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
    {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Stealth is not connected!");
    }

    if (IsInitialBlockDownload())
    {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Stealth is downloading blocks...");
    }
    ...
    if (params.size() == 0)
    {
        ...
        if (pindexBest && (pindexPrev != pindexBest ||
                           (nTransactionsUpdated != nTransactionsUpdatedLast &&
                            GetTime() - nStart > 60)))
        {
            if (pindexPrev != pindexBest)
        ...
        // deprecated
        result.push_back(
            Pair("midstate", HexStr(BEGIN(pmidstate), END(pmidstate))));
        result.push_back(Pair("data", HexStr(BEGIN(pdata), END(pdata))));
    ...
    else
    {
        ...
        pblock->nTime = pdata->nTime;
        pblock->nNonce = pdata->nNonce;
        pblock->vtx[0].vin[0].scriptSig = mapNewBlock[pdata->hashMerkleRoot]
                                              .second;
        pblock->hashMerkleRoot = pblock->BuildMerkleTree();
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
    {
        throw runtime_error(
            ...
            "transaction, including the generation award and transaction "
            "fees\n"
            "  \"target\" : hash target\n"
    ...
    if ((pindexPrev != pindexBest) ||
        ...
        ((GetFork(pindexBest->nHeight + 1) >= XST_FORK005) &&
         // give it 12 seconds for a new getwork (because future drift is only
         // 15 s)
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx)
    {
        ...
        if (tx.FetchInputs(txdb, mapUnused, false, false, mapInputs, fInvalid))
        {
            entry.push_back(
                Pair("fee",
                     (int64_t) (tx.GetValueIn(mapInputs) - tx.GetValueOut())));
    ...
    result.push_back(
        Pair("sigoplimit", (int64_t) chainParams.MAX_BLOCK_SIGOPS));
    result.push_back(Pair("sizelimit", (int64_t) chainParams.MAX_BLOCK_SIZE));
