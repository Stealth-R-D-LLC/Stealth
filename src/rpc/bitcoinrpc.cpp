...
{
    //                                   testnet : rpc
    return GetBoolArg("-testnet", false) ? chainParams.DEFAULT_RPCPORT_TESTNET :
                                           chainParams.DEFAULT_RPCPORT_MAINNET;
}
...
int64_t AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > chainParams.MAX_MONEY)
...
{ //  name                        function                    safemd  unlocked
  //  ------------------------    -----------------------     ------  --------
    ...
    { "createfeework",            &createfeework,             false,  true  },
    ...
    { "liststakerunspent",        &liststakerunspent,         false,  false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: StealthCoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use StealthCoind";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "sendtoaddress"          && n > 5) ConvertTo<Array>(params[5]);
    ...
    if (strMethod == "getbestblock"           && n > 0) ConvertTo<bool>(params[0]);
    ...
    if (strMethod == "liststakerunspent"      && n > 2) ConvertTo<boost::int64_t>(params[2]);
    if (strMethod == "liststakerunspent"      && n > 3) ConvertTo<boost::int64_t>(params[3]);
    ...
    if (strMethod == "getaddressoutputs"        && n > 2)
                                ConvertTo<boost::int64_t>(params[2]);
    if (strMethod == "getaddresstxspg"       && n > 1)
                                ConvertTo<boost::int64_t>(params[1]);
    if (strMethod == "getaddresstxspg"       && n > 2)
                                ConvertTo<boost::int64_t>(params[2]);
    if (strMethod == "getaddresstxspg"       && n > 3)
    ...
    if (strMethod == "getrichlist"              && n > 1)
                                ConvertTo<boost::int64_t>(params[1]);
    if (strMethod == "getrichlistpg"            && n > 0)
                                ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "getrichlistpg"            && n > 1)
                                ConvertTo<boost::int64_t>(params[1]);
    if (strMethod == "getrichlistpg"            && n > 2)
