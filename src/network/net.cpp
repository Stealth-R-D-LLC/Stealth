...
void CNode::PushGetBlocks(CBlockMemIndex* pmemIndexBegin, uint256 hashEnd)
{
    if (pmemIndexBegin == nullptr)
    {
        printf("PushGetBlocks() : TSNH null begin index\n");
        return;
    }
    // Filter out duplicate requests less than 30 seconds since last
    if (pmemIndexBegin == pmemIndexLastGetBlocksBegin &&
    ...
    pmemIndexLastGetBlocksBegin = pmemIndexBegin;
    hashLastGetBlocksEnd = hashEnd;
    if (fDebugNet)
    {
        printf("PushGetBlocks(): %s:\n    %s to \n    %s\n",
               addrName.c_str(),
               pmemIndexBegin->phashBlock->ToString().c_str(),
               hashEnd.ToString().c_str());
    }
    CBlockLocator locator(pmemIndexBegin);
    ...
    while (true)
    {
        ...
        //
        {
            ...
            BOOST_FOREACH (CNode* pnode, vNodesCopy)
            {
                     ...
                     pnode->vSend.empty()))
                {
                    ...
                    pnode->nReleaseTime = max(pnode->nReleaseTime,
                                              GetTime() + 15 * 60);
            ...
            if ((secured > 0) && (secured + unsecured > MINCONNREMODEL) &&
        ...
        BOOST_FOREACH (SOCKET hListenSocket, vhListenSocket)
        {
                ...
                FD_ISSET(hListenSocket, &fdsetRecv))
            {
                ...
                SOCKET hSocket = accept(hListenSocket,
                                        (struct sockaddr*) &sockaddr,
                                        &len);
                CAddress addr;
                int nInbound = 0;
                ...
                FD_ISSET(pnode->hSocket, &fdsetError))
            {
                ...
                if (lockRecv)
                {
                    ...
                    if (nPos > ReceiveBufferSize())
                    {
                        if (!pnode->fDisconnect)
                        {
                            printf("socket recv flood control disconnect "
                                   "(%" PRIszu " bytes)\n",
                                   vRecv.size());
                        }
                        pnode->CloseSocketDisconnect();
                    ...
                    else
                    {
                        ...
                        else if (nBytes < 0)
                        {
                            ...
                            int nErr = WSAGetLastError();
                            if (nErr != WSAEWOULDBLOCK &&
                                nErr != WSAEMSGSIZE && nErr != WSAEINTR &&
                                nErr != WSAEINPROGRESS)
                            {
                                if (!pnode->fDisconnect)
                                {
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    static const char*(*strDNSSeed)[2] = fTestNet ? strTestNetDNSSeed
                                                  : strMainNetDNSSeed;
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    while (true)
    {
        ...
        // entries can have many IPs if fNameLookup)
        {
            ...
            BOOST_FOREACH (CNode* pnode, vNodes)
            {
                for (vector<vector<CService>>::iterator it =
                         vservConnectAddresses.begin();
        ...
        BOOST_FOREACH (vector<CService>& vserv, vservConnectAddresses)
        {
            ...
            {
                printf(
                    "ThreadOpenAddedConnections: certified node:\n  \"%s\"\n",
                    addrToConnect.ToStringIPPort().c_str());
            }
            if (fDebugNet)
            {
...
void ThreadMessageHandler2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if ((GetFork(nHeight) >= XST_FORKQPOS) &&
            ((nMaxHeight <= 0) || (nBestHeight < nMaxHeight)) && fQPoSActive &&
            (fTestNet ||
             (GetBoolArg("-stake", true) && GetBoolArg("-staking", true))) &&
            !pwallet->IsLocked())
        {
           /*******************************************************************
            ** qPoS
...
bool BindListenPort(const CService& addrBind, string& strError)
{
...
#ifdef USE_IPV6
    // some systems don't have IPV6_V6ONLY but are always v6only; others do
    // have the option and enable it by default or not. Try to enable it, if
    // possible.
    if (addrBind.IsIPv6())
    {
#ifdef IPV6_V6ONLY
        ...
        SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
        {
            strError = strprintf(_("Unable to bind to %s on this computer. "
                                   "Stealth is probably already running."),
                                 addrBind.ToString().c_str());
        }
        else
        {
            strError = strprintf(_("Unable to bind to %s on this computer "
                                   "(bind returned error %d, %s)"),
                                 addrBind.ToString().c_str(),
                                 nErr,
                                 strerror(nErr));
        }
    ...
    if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR)
    {
        strError = strprintf("Error: Listening for incoming connections "
                             "failed (listen returned error %d)",
                             WSAGetLastError());
        printf("%s\n", strError.c_str());
        return false;
    }
...
static void run_tor() {
    ...
    string logDecl = "notice file " + GetDataDir().string() +
                          "/tor/tor.log";
...
bool StopNode()
{
    ...
    {
        printf("ThreadStealthMiner still running\n");
    }
#endif
    ...
    if (vnThreadsRunning[THREAD_STAKEMINTER] > 0)
    {
