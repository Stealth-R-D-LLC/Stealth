...
//////////////////////////////////////////////////////////////////////////////
/** Adds the coin hidden service. */
int add_coin_hs(const or_options_t *options, smartlist_t *service_list)
{
  tor_assert(options);
  tor_assert(service_list);

  int ret;
  hs_service_t *service = hs_service_new(options);

  /* Service Config */
  hs_service_config_t *coin_cfg = &service->config;
  coin_cfg->version = HS_VERSION_THREE;
  coin_cfg->directory_path = tor_strdup(coin_service_directory());

  /* Port */
  hs_port_config_t* coin_port = tor_malloc_zero(sizeof(hs_port_config_t) + 1);
  // p2p_port is declared in toradapter.h and defined in init.cpp
  coin_port->virtual_port = p2p_port;
  coin_port->real_port = p2p_port;
  coin_port->is_unix_addr = 0;
  coin_port->real_addr.family = AF_INET;

  smartlist_add(coin_cfg->ports, coin_port);

  tor_inet_aton("127.0.0.1", &(coin_port->real_addr.addr.in_addr));

  ret = hs_check_service_private_dir(options->User,
                                     service->config.directory_path,
                                     service->config.dir_group_readable,
                                     0);
  if (ret < 0)
...
hs_config_service_all(const or_options_t *options, int validate_only)
{
  ...
  /* Do this first to preempt services added by any config file. */
  add_coin_hs(options, new_service_list);
